...
> **Status: LOCKED 2026-09-18.** House rules transcribed from the user and amended with his 2026-09-18 rulings (marked where they supersede the original transcription). Settled engineering/product decisions live in [DESIGN_DECISIONS.md](DESIGN_DECISIONS.md) — do not re-litigate there. This file is the product contract; [PROGRESS.md](PROGRESS.md) tracks state.
...
**Purpose: a single-device web app ("GM cockpit") that replaces the cards and the bookkeeping — secret deal, night phase, round timers, randomized voting order with live tallies and vote attribution, tie-breaks, elimination reveals, win detection — so the GM only referees people, not state.**

## Project non-negotiables

1. **The app must never leak a secret role.** During group-visible phases the screen shows nothing role-related; the secret deal uses hold-to-peek with auto-advance. Leak-proofing is a hard rule with explicit tests.
...
- **Role assignment (variable count)** — physical cards are abandoned; the app assigns identities:
  ...
  - **Sheriff toggle** (any size): adds a sheriff in place of one civilian — "sometimes we add a sheriff". At 9–11 the single special slot is the GM's choice of Detective **or** Sheriff (never both by default).
...
- **Tie procedure:** tied players get a 30-second defense period, then all OTHER players (tied excluded) vote to eliminate exactly one of the tied. **If the runoff also ties, the runoff repeats among the still-tied (still excluding the tied from voting) until someone is eliminated** *(ruling 2026-09-18: revote until broken)*.
...
- **Deal is secret**: pass-the-GM's-phone reveal; each player holds-to-peek their **role** (text only — no card faces; cards are fully abandoned) and the screen auto-advances to the next player.
...
1. **Setup** — roster (names), variant toggle (roleless / roles), player count 7–15 with the role table auto-applied (mafia override ±1, sheriff toggle, detective-or-sheriff pick at 9–11), timer lengths (defaults 5:00 discussion / 0:30 defense), random deal, **pass-the-phone role reveal** (hold-to-peek, role text only, auto-advance).
2. **Game board** — seat list with alive/dead, round counter, mafia-vs-town parity counter.
3. **Day runner** — 5:00 discussion timer (pause/resume, chime at zero), then voting mode: randomized order shown player-by-player, GM taps the accused per voter, **live tally + who-voted-for-whom**, automatic plurality elimination, tie flow (30s defense timer → runoff excluding tied → repeat runoff until broken), reveal confirmation, win check.
4. **Night wizard** (roles variant) — guided fixed-order prompts; GM records mafia target, sheriff target, detective guess + private thumbs UI, doctor target; engine resolves; **morning announcement screen** (deaths by name + doctor save announcement when one occurred, suggested script included).
5. **End screen** — victory banner, **full role reveal**, one-tap new game.
6. **Rules reference** — the house rules formatted for teaching new players.
7. **Persistence** — full state snapshot to localStorage after every action; reload/crash resumes mid-game. Installable PWA; fully offline.
...
3. **Setup flow + secret deal** (leak-proof role reveal).
...
All open items resolved by the user: GM-only single device; variable 7–15 players with the table above; sheriff omission = role absent; full game-end reveals; runoff deadlock = revote until broken; randomized voting order (supersedes rotation); live voting with attribution (supersedes sequential order); engine defaults adopted (detective guesses nightly, doctor repeats incl. self, mafia must kill nightly, timers configurable); GitHub Pages confirmed; doctor save success announced publicly (correction to original transcription); reveals show role text only (cards abandoned).